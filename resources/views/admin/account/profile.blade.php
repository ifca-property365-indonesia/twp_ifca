...
<div class="row g-4">
    <div class="col-md-5">
        ...
        <div class="text-center" id="avatarStage">
            <img id="picturebox" class="avatar-preview mb-3 pictured" src="{{ url('images/default/defaultUser.png') }}" onerror="{{ \App\Support\ProfilePicture::onError() }}" alt="{{ __('admin/account.profile_picture') }}">
            <div class="mb-2">
                <label for="userfile" class="btn btn-outline-primary btn-sm"><i class="cil-cloud-upload"></i><span>{{ __('admin/account.change_picture') }}</span></label>
                <input type="file" id="userfile" name="userfile" class="d-none" accept="image/png,image/jpeg,image/gif">
            </div>
            <div class="form-note">{{ __('admin/account.picture_note') }}</div>
            <div class="form-note text-primary d-none" id="pictureHint"><i class="cil-info"></i> {!! __('admin/account.picture_hint') !!}</div>
        ...
        <div class="text-center d-none" id="cropStage">
            ...
            <div class="form-note mt-2">{{ __('admin/account.crop_help') }}</div>
            <div class="d-flex justify-content-center gap-2 mt-3">
                <button type="button" class="btn btn-sm btn-secondary" id="btnCropCancel">{{ __('common.cancel') }}</button>
                <button type="button" class="btn btn-sm btn-primary" id="btnCropApply"><i class="cil-check"></i><span>{{ __('admin/account.use_photo') }}</span></button>
            </div>
        </div>
    </div>
    ...
    <div class="col-md-7">
        <ul class="nav nav-underline-border mb-3" role="tablist">
            <li class="nav-item">
                <a class="nav-link active" data-coreui-toggle="tab" href="#tabPersonal" role="tab"><i class="cil-user"></i> {{ __('admin/account.personal') }}</a>
            </li>
            <li class="nav-item">
        ...
        <div class="tab-content">
            <div class="tab-pane active" id="tabPersonal" role="tabpanel">
                <form id="frmEditor" method="post" action="" novalidate>
                    ...
                    <div class="mb-3">
                        <label for="handphone" class="form-label">{{ __('admin/account.handphone') }} <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" id="handphone" name="handphone">
                        <div class="form-note">{{ __('admin/account.phone_format') }}</div>
                    </div>
            ...
            <div class="tab-pane" id="tabPassword" role="tabpanel">
                <form id="frmchangepass" method="post" action="" novalidate>
                    @csrf
                    <div class="mb-3">
                        <label for="password1" class="form-label">{{ __('admin/account.new_password') }} <span class="text-danger">*</span></label>
                        <input type="password" class="form-control" id="password1" name="password1" required autocomplete="new-password">
                    </div>
                    <div class="mb-3">
                        <label for="password2" class="form-label">{{ __('admin/account.confirm_password') }} <span class="text-danger">*</span></label>
...
(function ($) {
    ...
    }, @json(__('admin/account.password_mismatch')));
    ...
    $('#userfile').on('change', function () {
        ...
        if (!/^image\/(png|jpe?g|gif)$/i.test(file.type)) {
            Swal.fire({ title: @json(__('common.information')), text: @json(__('admin/account.only_image')), icon: 'warning' });
            this.value = '';
            return;
        }
        if (file.size > 5000000) {
            Swal.fire({ title: @json(__('common.information')), text: @json(__('admin/account.max_size')), icon: 'warning' });
            this.value = '';
            return;
        }
    ...
    $('#btnSave').on('click', function () {
        ...
        if (cropper) {
            Swal.fire({
                title: @json(__('admin/account.picture_not_applied')),
                text: @json(__('admin/account.picture_not_applied_text')),
                icon: 'warning',
